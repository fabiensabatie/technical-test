...
class ObservableV2 {
   ...
   */
  on (name, f) {
    setIfUndefined$1(this._observers, /** @type {string} */ (name), create$8).add(f);
    return f
  }
...
/* c8 ignore next */
const utf8TextEncoder$2 = /** @type {TextEncoder} */ (typeof TextEncoder !== 'undefined' ? new TextEncoder() : null);
 ...
 */
const _strBuffer$2 = new Uint8Array(30000);
 ...
 */
/* c8 ignore next */
const writeVarString$2 = (utf8TextEncoder$2 && /** @type {any} */ (utf8TextEncoder$2).encodeInto) ? _writeVarStringNative$2 : _writeVarStringPolyfill$2;
 ...
 * @return {String} The read String
 */
const _readVarStringNative$2 = decoder =>
  /** @type any */ (utf8TextDecoder$2).decode(readVarUint8Array$2(decoder));
 ...
 * @return {Promise<T>}
 */
const create$6 = f => /** @type {Promise<T>} */ (new Promise(f));
 ...
 * @return {boolean}
 */
const equalFlat = (a, b) => a === b || (size(a) === size(b) && every(a, (val, key) => (val !== undefined || hasProperty$1(b, key)) && b[key] === val));
...
const computeParams = () => {
  if (params === undefined) {
    if (isNode) {
      ...
      params = create$9(); // eslint-disable-next-line no-undef
...
const USER_ID_KEY = "mexty_realtime_user_id";
function getUserId() {
...
function useCollabSpace(documentName, initialState, options = {}) {
    ...
    const websocketUrl = options.websocketUrl || `wss://collab.mexty.ai`;
    // Initialize Yjs document and provider
    react.useEffect(() => {
        console.log(`🔄 Initializing collaboration for document: ${documentName}`);
        console.log(`🔗 Connecting to: ${websocketUrl}`);
        // Create Y.Doc
        const doc = new Doc();
        docRef.current = doc;
        // Get or create the shared map
        const yMap = doc.getMap("state");
        yMapRef.current = yMap;
        // Initialize with initial state if the map is empty
        if (yMap.size === 0) {
            Object.entries(initialState).forEach(([key, value]) => {
                yMap.set(key, value);
            });
        }
        // Set initial state from Y.Map
        const currentState = {};
        ...
        const provider = new HocuspocusProvider({
            ...
            document: doc,
            onOpen: () => {
                console.log(`🔌 WebSocket opened for: ${documentName}`);
                setConnectionStatus("connecting");
            },
            onConnect: () => {
                var _a;
                console.log(`✅ Connected to collaboration server for: ${documentName}`);
                setIsConnected(true);
            ...
            onDisconnect: ({ event }) => {
                var _a;
                console.log(`❌ Disconnected from collaboration server for: ${documentName}`, event);
                setIsConnected(false);
                setConnectionStatus("disconnected");
                (_a = options.onDisconnect) === null || _a === void 0 ? void 0 : _a.call(options);
            },
            ...
            onSynced: () => {
                ...
                // Update state when synced with server
                const syncedState = {};
                yMap.forEach((value, key) => {
                    syncedState[key] = value;
                });
        ...
        setConnectionStatus("connecting");
        // Listen to changes in the Y.Map
    ...
    }, [documentName, websocketUrl]);
    const update = react.useCallback((updateFn) => {
        if (!yMapRef.current)
            return;
        const yMap = yMapRef.current;
        // Get current state from Y.Map
        const currentState = {};
        yMap.forEach((value, key) => {
            currentState[key] = value;
        });
        // Calculate the update
        const updateData = typeof updateFn === "function" ? updateFn(currentState) : updateFn;
...
exports.useCollabSpace = useCollabSpace;
